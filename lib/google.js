...
var URL = '%s://www.google.%s/search?hl=%s&q=%s&uule=%s&sa=N&num=%s&ie=UTF-8&oe=UTF-8&gws_rd=ssl&gl=%s&start=%s';
...
function google(query, sedb, lang, start, proxy, callback) {
    ...
    igoogle(query, sedb, lang, start, proxy, callback);
}
...
//                    logger.alert('Next page');
                    nextFunc = function() {
//                        console.log('start2',start + parseInt(google.resultsPerPage))
                        igoogle(query, sedb, lang, start + parseInt(google.resultsPerPage), proxy, callback)
                    }
                }
